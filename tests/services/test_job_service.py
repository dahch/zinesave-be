...
async def test_create_job_duplicate(mock_getaddrinfo, job_service, job_repo):
    ...
    job_repo.get_recent_duplicate.return_value = Job(id="123", source_url="https://example.com/article")
...
@pytest.mark.asyncio
@patch("app.services.job_service.socket.getaddrinfo")
async def test_create_job_success(mock_getaddrinfo, job_service, job_repo, user_repo, queue_service):
    mock_getaddrinfo.return_value = [(2, 1, 6, '', ('93.184.216.34', 0))]
    user = User(id=1, email="test@example.com", credits=10)
