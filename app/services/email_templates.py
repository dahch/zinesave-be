...
def create_reset_password_email(reset_link: str, frontend_url: str):
    ...
    """
    # Inline Styles (reusing verification email styles for consistency)
    style_body = f"margin: 0; padding: 0; width: 100%; background-color: {COLOR_BG_BODY}; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%;"
    style_wrapper = f"width: 100%; background-color: {COLOR_BG_BODY}; padding: 40px 0;"
    style_table = f"margin: 0 auto; max-width: 600px; width: 100%; background-color: {COLOR_BG_CARD}; border-radius: 8px; overflow: hidden; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;"
    style_header = "padding: 24px; text-align: center; background-color: #ffffff; border-bottom: 1px solid #f0f0f0;"
    ...
    <center style="width: 100%; background-color: {COLOR_BG_BODY};">
        <div class="wrapper" style="{style_wrapper}">
            ...
            <table class="main-table" align="center" border="0" cellpadding="0" cellspacing="0" role="presentation" style="{style_table}">
                ...
                <tr>
                    <td class="content" style="{style_content}">
                        <h1 style="{style_h1}">Reset Your Password</h1>
                        <p style="{style_p}">We received a request to reset your password. Click the button below to choose a new one:</p>
                        
                        <div class="button-wrapper" style="text-align: center; margin: 32px 0;">
                            <a href="{reset_link}" class="button" style="{style_button}">Reset Password</a>
                        </div>
                        
                        <p style="{style_p}; font-size: 14px; color: {COLOR_TEXT_MUTED}; margin-top: 24px;">
                            If the button doesn't work, copy and paste this link into your browser:<br>
                            <a href="{reset_link}" style="{style_link}">{reset_link}</a>
                        </p>
...
def create_welcome_email(frontend_url: str, contact_email: str):
    """
    Creates the HTML content for the welcome email (e.g. for OAuth users).
    """
    # Inline Styles (reusing styles for consistency)
    style_body = f"margin: 0; padding: 0; width: 100%; background-color: {COLOR_BG_BODY}; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%;"
    style_wrapper = f"width: 100%; background-color: {COLOR_BG_BODY}; padding: 40px 0;"
    style_table = f"margin: 0 auto; max-width: 600px; width: 100%; background-color: {COLOR_BG_CARD}; border-radius: 8px; overflow: hidden; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;"
    style_header = "padding: 24px; text-align: center; background-color: #ffffff; border-bottom: 1px solid #f0f0f0;"
...
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Welcome to ZineSave</title>
    {get_base_styles()}
</head>
<body style="{style_body}">
    <center style="width: 100%; background-color: {COLOR_BG_BODY};">
        <div class="wrapper" style="{style_wrapper}">
            ...
            <table class="main-table" align="center" border="0" cellpadding="0" cellspacing="0" role="presentation" style="{style_table}">
                ...
                <tr>
                    <td class="content" style="{style_content}">
                        <h1 style="{style_h1}">Welcome to ZineSave!</h1>
                        <p style="{style_p}">We're thrilled to have you on board. ZineSave is the best way to turn the web into beautiful ePubs.</p>
                        <p style="{style_p}">You can now start saving your favorite articles and building your collection.</p>
                        
                        <div class="button-wrapper" style="text-align: center; margin: 32px 0;">
                            <a href="{dashboard_link}" class="button" style="{style_button}">Go to Dashboard</a>
                        </div>
                        
                        <p style="{style_p}; margin-top: 32px; color: {COLOR_TEXT_MUTED};">
                            If you have any questions, feel free to reach out to us at <a href="mailto:{contact_email}" style="{style_link}">{contact_email}</a>. We're here to help!
                        </p>
                    </td>
                </tr>
            </table>
