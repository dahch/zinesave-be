...
def fetch_and_extract(url: str) -> dict:
    ...
    return {"title": title, "html": cleaned_html, "metadata": metadata}
...
def extract_content(db: Session, job: Job):
    ...
    meta_content = JobContent(
        job_id=job.id, step="metadata", content_type="json", content=json.dumps(result["metadata"])
    )
    db.add(meta_content)

    # Save extracted content
    content = JobContent(
        job_id=job.id, step="extracted", content_type="html", content=result["html"]
    )
    db.add(content)
