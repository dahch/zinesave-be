...
class UserService:
    def __init__(self, user_repo: UserRepository, job_repo: JobRepository, cloud_conn_repo: CloudConnectionRepository):
        self.user_repo = user_repo
        self.job_repo = job_repo
        self.cloud_conn_repo = cloud_conn_repo

    def get_me(self, user: User) -> Dict[str, Any]:
        connections = self.cloud_conn_repo.db.query(
            self.cloud_conn_repo.model.provider
        ).filter(self.cloud_conn_repo.model.user_id == user.id).all()
